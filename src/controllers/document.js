...
const sendResponse = (dbOperation, req, res, next) => {
	dbOperation
		.then(data => {
			if (req.query.ContentType === 'ejson') data = JSON.stringify(EJSON.serialize(data));
			res.send(data);
		})
		.catch(err => next(err));
}
...
function middleware(req, res, next) {
	req.body = (req.query.incomingType === 'ejson') ? EJSON.deserialize(req.body) : req.body;
...
function count(req, res, next) {
	...
	dbOperation
		.then(data => {
			...
			if (req.query.ContentType === 'ejson') data = JSON.stringify(EJSON.serialize(
				data
			));
			res.send(data);
		})
